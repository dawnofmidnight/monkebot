...
class Bot(commands.Bot):
    ...
    async def startup_greeting(self) -> None:
        """Announce presence to the general channel."""
        embed = Embed(description="Connected!")
        embed.set_author(
            name="monkebot", url=constants.BOT_REPO_URL, icon_url=self.user.avatar_url
        )
        await self.get_channel(constants.Channels.general).send(embed=embed)
